// 小报童文章下载器 v3.1.5 - 最终修复版本
// 基于v3.1版本，只修复ZIP生成问题，保持数据获取逻辑不变
...
function initializeDownloader() {
  console.log("🚀 小报童下载器 v3.1.5 开始加载...");
...
function createDownloadUI() {
  ...
  ui.innerHTML = `
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; border-bottom: 1px solid #eee; padding-bottom: 8px;">
      <div style="font-weight: bold; color: #007cba; font-size: 16px;">📥 小报童下载器 v3.1.5</div>
    ...
    <div style="font-size: 11px; color: #666; margin-top: 8px;">
      <strong>说明：</strong>先手动滚动到页面底部，再点击开始抓取。v3.1.5修复大数据量ZIP生成问题。
    </div>
  `;
...
function sanitizeFileName(fileName) {
  ...
  // 安全的替换操作
  try {
    return safeFileName.replace(/[<>:"/\\|?*]/g, '_').substring(0, 50);
  } catch (error) {
  ...
  try {
    ...
    apiData.forEach((article, index) => {
      try {
        ...
        console.error(`处理第${index + 1}篇文章时出错:`, error);
        // 继续处理其他文章，不中断整个流程
      }
    });
